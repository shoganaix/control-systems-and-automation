%% FBa-08 - FIGURAS FINALES (G(t) vs G(f))  [2º ORDEN]
% Este script NO modifica figuras anteriores
clear; close all; clc;

%% 0) Archivo
fileWave = 'FBa-08_Waveform.csv';
...
xlabel('Tiempo (s)');
ylabel('Salida (V)');
title('Comparación a Escalón (FBa-08): Laboratorio vs G(t)/G(f)');
...
ylabel('Ganancia (dB)');
title('Diagrama de Bode - Módulo (FBa-08): Laboratorio vs G(t)/G(f)');
...
xlabel('Frecuencia (Hz)');
ylabel('Fase (°)');
title('Diagrama de Bode - Fase (FBa-08): G(t) vs G(f)');
