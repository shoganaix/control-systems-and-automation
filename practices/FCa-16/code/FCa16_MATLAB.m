%% FCa-16 - Practica Bloque I
clear; close all; clc;

%% 0) Archivos
fileWave = 'FCa-16_Waveform.csv';      % escalón (t, CH1, CH2)
...
ylabel('Ganancia (dB)');
ylim padded
title('Diagrama de Bode - Módulo (FCa-16)');
...
xlabel('Frecuencia (Hz)');
ylabel('Fase (°)');
title('Diagrama de Bode - Fase (FCa-16)');

%% 12) Bode anotado para 2º orden (FCa-16) [Bode + fase, marca Mr 𝑓r]
...
xlabel('Frecuencia (Hz)');
ylabel('Ganancia (dB)');
title('Diagrama de Bode - Módulo (FCa-16) [Anotado]');
if exist('f_exp','var') && exist('Gexp_dB','var')
...
xlabel('Frecuencia (Hz)');
ylabel('Fase (°)');
title('Diagrama de Bode - Fase (FCa-16) [Anotado]');
legend('Modelo (tf)', 'Location', 'best');
